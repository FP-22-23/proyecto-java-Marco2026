...
public class FactoriaCompras {
	...
	public static Compra parsearCompra(String lineaCSV) {
		...
		Purchase purchase = new Purchase(Integer.valueOf(campos[2].trim()), Double.valueOf(campos[3].trim()));
		LocalDateTime purchaseDate = LocalDateTime.parse(campos[4].trim(), DateTimeFormatter.ofPattern("dd/MM/yyyy hh:mm"));
		Integer customerId = Integer.valueOf(campos[5].trim());
		TypeCountry country = TypeCountry.valueOf(campos[6].trim().toUpperCase());
